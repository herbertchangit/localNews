...
export default function ArticleDetail({ preview = false }: { preview?: boolean }) {
  ...
  if (loading) return <>{!preview && <ReaderHeader />}<main className="articleState">Loading full story… / 正在載入完整新聞…</main></>;
  if (!article || error) return <>{!preview && <ReaderHeader />}<main className="articleState"><h1>Story not found / 找不到新聞</h1><p>{error}</p><Link to={backTo}><ArrowLeft />{preview ? "Back to Story Management / 返回新聞管理" : "Back to local news / 返回本地新聞"}</Link></main></>;
  ...
  return <div className="articlePage">{!preview && <ReaderHeader />}<main className="articleMain">
    ...
    <article>
      <header className="articleHeading">
        ...
        {!preview && <ShareStoryButton title={article.title} slug={article.slug} previewImage={photos[0]?.url} className="articleShareButton" />}
      </header>
