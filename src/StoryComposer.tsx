...
export const openStoryComposer = () =>
  window.dispatchEvent(new Event(OPEN_STORY_COMPOSER_EVENT));
...
export default function StoryComposer() {
  ...
  useEffect(() => {
    ...
    if (
      !canCreate ||
      !["/newsroom", "/newsroom/stories"].includes(location.pathname)
    )
      return;
  ...
  return (
    <>
      ...
      {notice &&
        createPortal(
          <div className="storyComposerNotice">
            <span>Draft saved:</span> {notice}
...
function StoryModal({
  ...
  const choosePhotos = async (files?: FileList | null) => {
    ...
    if (photos.length + incoming.length > 12)
      return setError(
        "A story can have up to 12 photos / 每篇新聞最多可有 12 張照片",
      );
    for (const file of incoming) {
      if (!/^image\/(png|jpeg|webp)$/.test(file.type))
        return setError(
          "Use PNG, JPEG or WebP photos / 請使用 PNG、JPEG 或 WebP 照片",
        );
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (richTextToPlainText(form.excerpt).length < 20)
      return setError("Summary must contain at least 20 characters.");
    if (richTextToPlainText(form.excerpt).length > 600)
      return setError("Summary must contain no more than 600 characters.");
    if (richTextToPlainText(form.content).length < 40)
      return setError("Story content must contain at least 40 characters.");
    setBusy(true);
    setError("");
    try {
      ...
      for (const photo of photos) {
        const photoResponse = await fetch(
          `/api/newsroom/articles/${story.id}/photos`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
        ...
        if (!photoResponse.ok)
          throw new Error(
            photoResult.error || "Draft saved, but a photo upload failed",
          );
      }
    ...
    <div className="modalBackdrop">
      <form className="userModal storyComposerModal" onSubmit={submit}>
        <div className="modalHead">
          <div>
            <small>NEW STORY · 新增新聞</small>
            <h2>Create story draft / 建立新聞草稿</h2>
          </div>
          <button type="button" onClick={onClose}>
            <X />
          </button>
        </div>
        <div className="storyComposerIntro">
          <FileText />
          <p>
            Start a draft for editorial review. It will not be published
            automatically.
          </p>
        </div>
