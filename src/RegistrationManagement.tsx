...
import { CalendarDays, ClipboardList, Copy, Eye, FileText, LayoutDashboard, Pencil, Plus, Settings, Trash2, Users, X } from "lucide-react";
...
const empty = { eventName: "", description: "", active: true, eventDates: [""] };
...
export default function RegistrationManagement() {
  ...
  useEffect(() => { load(); }, []);
  const open = (form?: RegistrationForm) => setEditor(form ? { ...form, eventDates: form.eventDates.map((item) => day(item.eventDate)) } : { ...empty, eventDates: [...empty.eventDates] });
  const save = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const saved = await api(`/api/registrations/admin/forms${editing ? `/${editor.id}` : ""}`, { method: editing ? "PATCH" : "POST", body: JSON.stringify(body) });
  ...
  return <div className="dash registrationAdmin">
    ...
    {editor && <div className="modalBackdrop" onMouseDown={() => setEditor(null)}><form className="registrationEditor" onSubmit={save} onMouseDown={(event) => event.stopPropagation()}><div className="modalHead"><div><small>{editor.id ? "EDIT FORM" : "NEW FORM"}</small><h2>{editor.id ? "Edit registration form" : "Create registration form"}</h2></div><button type="button" onClick={() => setEditor(null)}><X /></button></div><label>Event name<input required minLength={2} maxLength={160} value={editor.eventName} onChange={(event) => setEditor({ ...editor, eventName: event.target.value })} /></label><label>Description<textarea required minLength={2} maxLength={5000} rows={5} value={editor.description} onChange={(event) => setEditor({ ...editor, description: event.target.value })} /></label><fieldset><legend>Event dates</legend>{editor.eventDates.map((value: string, index: number) => <div className="registrationDateInput" key={index}><input type="date" required value={value} onChange={(event) => setEditor({ ...editor, eventDates: editor.eventDates.map((item: string, itemIndex: number) => itemIndex === index ? event.target.value : item) })} /><button type="button" disabled={editor.eventDates.length === 1} onClick={() => setEditor({ ...editor, eventDates: editor.eventDates.filter((_: string, itemIndex: number) => itemIndex !== index) })}><Trash2 /></button></div>)}<button className="addDate" type="button" onClick={() => setEditor({ ...editor, eventDates: [...editor.eventDates, ""] })}><Plus />Add another date</button></fieldset><label className="registrationSwitch"><input type="checkbox" checked={editor.active} onChange={(event) => setEditor({ ...editor, active: event.target.checked })} /><span />Public form is open</label><div className="modalActions"><button type="button" onClick={() => setEditor(null)}>Cancel</button><button className="new" type="submit">{editor.id ? "Save changes" : "Create form"}</button></div></form></div>}
