...
import { CalendarDays, ClipboardList, Copy, Eye, FileText, ImagePlus, LayoutDashboard, Pencil, Plus, Search, Settings, Trash2, Users, X } from "lucide-react";
...
export default function RegistrationManagement() {
  ...
  const [canManage, setCanManage] = useState(false), [viewerOptions, setViewerOptions] = useState<Viewer[]>([]), [hideUnregistered, setHideUnregistered] = useState(true);
  ...
  const visibleSubmissions = detail?.submissions.filter((submission) => !hideUnregistered || !submission.unregisteredAt) || [];
  const filteredViewerOptions = viewerOptions.filter((viewer) => `${viewer.name} ${viewer.email} ${viewer.role || ""}`.toLowerCase().includes(viewerQuery.trim().toLowerCase()));
  ...
  return <div className="dash registrationAdmin">
    <aside><Link to="/" className="brand light"><span>LN</span><div>LOCAL NEWS<small>NEWSROOM OS</small></div></Link><div className="workspace"><small>WORKSPACE</small><b>{current?.user?.name}</b></div><button data-session-common="true" onClick={() => nav("/newsroom")}><LayoutDashboard />Overview</button><button data-session-common="true"><FileText />Stories</button><button data-session-common="true"><Users />People</button><button data-session-common="true" className="active"><ClipboardList />Registration</button><button data-session-common="true"><Settings />Settings</button><div className="profile"><div>{initials}</div><span><b>{current?.user?.name}</b><small>{current?.user?.role}</small></span></div></aside>
    <section className="content registrationContent"><div className="top"><div><small>ADMINISTRATION / REGISTRATION · 管理 / 登记</small><h1>Registration forms / 登记表格</h1><p>{canManage ? "Create event forms, share public links and review pre-registrations." : "View registrations for forms assigned to you."}</p></div><button className="new" disabled={!canManage} onClick={() => open()} title={canManage ? "Create registration form" : "View-only access"}><Plus />New form / 新建表格</button></div>
      ...
      <div className="registrationGrid">{loading && <div className="registrationEmpty">Loading registration forms…</div>}{!loading && !forms.length && <div className="registrationEmpty"><ClipboardList /><h2>{canManage ? "No registration forms yet" : "No registration forms assigned"}</h2><p>{canManage ? "Create the first form to receive outsider pre-registrations." : "Registration will appear here after an administrator assigns a form to you."}</p><button className="new" disabled={!canManage} onClick={() => open()}><Plus />New form</button></div>}{forms.map((form) => <article className="registrationCard" key={form.id}><div className="registrationCardHead"><span className={form.active ? "open" : "closed"}>{form.active ? "Open" : "Closed"}</span><small>{form._count.submissions} registrations</small></div><h2>{form.eventName}</h2><p>{form.description}</p><div className="registrationDates">{form.eventDates.map((item) => <span key={item.id}><CalendarDays />{new Date(item.eventDate).toLocaleDateString()}</span>)}</div><small>Created by {form.creator.name}</small><div className="registrationCardActions"><button onClick={() => showResponses(form)} title="View registrations"><Eye /></button><button disabled={!canManage} onClick={() => canManage && copyLink(form)} title={canManage ? "Copy public link" : "View-only access"}><Copy /></button><button disabled={!canManage} onClick={() => open(form)} title={canManage ? "Edit form" : "View-only access"}><Pencil /></button><button disabled={!canManage} className="danger" onClick={() => canManage && remove(form)} title={canManage ? "Delete form" : "View-only access"}><Trash2 /></button></div></article>)}</div>
    </section>
    ...
    {detail && <div className="modalBackdrop" onMouseDown={() => setDetail(null)}>
      <section className="registrationResponses" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <label className="registrationResponseFilter"><input type="checkbox" checked={hideUnregistered} onChange={(event) => setHideUnregistered(event.target.checked)} />Hide un-registered registrations / 隐藏已取消登记</label>
