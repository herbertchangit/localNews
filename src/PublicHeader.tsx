...
export default function PublicHeader({
  ...
  hideLoginWhenSignedOut = false,
  onMenu,
}: {
  children?: ReactNode;
  className?: string;
  hideSessionActions?: boolean;
  hideLoginWhenSignedOut?: boolean;
  onMenu?: () => void;
}) {
  const location = useLocation();
  const session = readHeaderSession();
  const jingSiLabel = session?.user.name?.trim() || "JingSi (靜思語)";
  ...
  return <><div className="ticker jingSiTicker" ref={tickerRef}><b>{jingSiLabel}</b><span>{jingSiMessage}</span></div><div className={`publicHeaderSticky${tickerHidden ? " tickerHidden" : ""}`}><header className={`publicSiteHeader ${className}`.trim()}>
